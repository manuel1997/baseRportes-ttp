...
@Component({
  selector: 'app-resumenatencionoagr-reporte',
  templateUrl: './resumenatencionoagr-reporte.component.html',
  styleUrls: ['./resumenatencionoagr-reporte.component.css']
})
export class ResumenatencionoagrReporteComponent implements OnInit {
  ...
  async mostrarReporte() {
    ...
    let data = await this._api.rgnoa_noagrResumenA();
    ...
    if (data['data'] == '' || data['data'] == undefined) {
      ...
      if(resultado == 'ok') {
        ...
        setTimeout(() => {
          ...
          tmpElemento.download = 'Resumen_Atencion_noagr_' + new Date().getTime() + EXCEL_EXTENSION;
          tmpElemento.click();
        } , 1200);
      } else {
        Swal('Error', this.validarService.validarError(data), 'error');
