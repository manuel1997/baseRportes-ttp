...
@Component({
  selector: 'app-rangoatencion-reporte',
  templateUrl: './rangoatencion-reporte.component.html',
  styleUrls: ['./rangoatencion-reporte.component.css']
})
export class RangoatencionReporteComponent implements OnInit {
  ...
  async mostrarReporte(){
    ...
    let data = await this._api.rgnoa_rangoatencion();
      ...
      // if (data.data == "error login" || data.data.includes("999")) {
        ...
        for (let index = 0; index < registros.length; index++) {
          ...
          if (index == 0) {
            ...
            this.datos.push({"oficina":campos[1], "serie": campos[2], "atendidos_n":campos[3], "atendidos_porcentaje": campos[4], "atencion_promedio": this.minuto(campos[5])});
          }
        }
      } else {
    ...
    if (data['data'] == '' || data['data'] == undefined) {
      ...
      if(resultado == 'ok'){
        ...
        setTimeout(() => {
          ...
          tmpElemento.download = 'Rango_Atencion_' + new Date().getTime() + EXCEL_EXTENSION;
          tmpElemento.click();
        } , 1200);
      } else {
